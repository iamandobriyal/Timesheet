...
export class ProfileComponent implements OnInit {
  ...
  public updatePassword() {
    ...
    if (newPassword.length > 0) {
      this.profileService.updatePassword(newPassword).subscribe((data) => {
        if (data.success) {
  ...
  public onSelectFile(event) {
    if (event.target.files && event.target.files[0]) {
      ...
      if (event.target.files[0].size < 5 * 1024 * 1024 + 1) {
        // max 5*1024KB
        reader.readAsDataURL(event.target.files[0]); // read file as data url
      } else {
        let size_ =
          Math.round((event.target.files[0].size / 1024 / 1024) * 10) / 10;
        let text =
          "Please select an image size smalller then 1MB. Founded size [" +
          size_ +
          " MB]";
        alert(text);
      }
      reader.onload = (event: Event) => {
        ...
        this.profileService
          .postProfileAvatar(base64_image)
          .subscribe((data) => {
            if (data.success) {
              alert(
                "Take a few seconds to be processed. (try relogin otherwise)"
              );
