...
async function updatePassword(req, res) {
  ...
  const salt = authCtrl.genRandomString(16);

  // Hash the password with the new salt
  const passwordData = authCtrl.sha512(password, salt);

  // Set the new salt and hashed password
  const nSalt = salt;
  const hashedPassword = bcrypt.hashSync(passwordData.passwordHash, 10);

  try {
    // Update the user with the new salt and hashed password
    const updatedUser = await User.findByIdAndUpdate(
      id,
      { nSalt: nSalt, hashedPassword: hashedPassword },
      { new: true }
    );

    res.status(200).json({ success: true, message: 'Password updated successfully' });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error updating password', error: error });
  }
}
