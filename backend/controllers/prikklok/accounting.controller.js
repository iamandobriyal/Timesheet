...
async function project_task(req, res) {
  let projects = await Project.aggregate([
    {
      $match: {
        assigned: { $in: [req.user._id.toString()] },
      },
    },
  ]);
  let project_task = [];
  for (let i = 0; i < projects.length; i++) {
    ...
    project_task.push(line);
  }
  res.json(project_task);
}

async function update_events(req, res) {
  const data = req.body;
  let events = data.events;
  let current_time = Date.now();
  let event_time = new Date(data.currentdate + 'T11:00:00.000Z');
  for (let i = 0; i < events.length; i++) {
    let event = events[i];
    let user_id = req.user._id.toString();
    let project_id = event.projectId;
    let task = event.task;
    ...
    if (eventId != "") {
      ...
      const updatedEvent = await Event.findByIdAndUpdate(eventId, obj);
    } else {
      // Create a new event
...
async function remove_event(req, res) {
  ...
  let user_assigned_tasks = await Task.aggregate([
    {
      $match: {
        _id: {
          $in: user_assigned_project[0].sub_tasks.map(mongoose.Types.ObjectId),
        },
      },
    },
    {
    ...
    user_assigned_tasks[index].clock_out_events = await Event.aggregate([
      {
        $match: {
          _id: {
            $in: user_assigned_tasks[index].clock_out_events.map(
      ...
      },
      {
        $project: {
          ...
          hour: {
            $sum: [{ $hour: { date: "$time_stop", timezone: "+0100" } }, 1],
          },
        },
      },
    ]);
      ...
      if (
        ...
        user_assigned_project[index].sub_tasks[j].clock_out_events =
          await Event.aggregate([
            ...
            {
              $project: {
                // make the object and get the week and hours of the event
                _id: 0,
                user_id: 1,
                week: { $sum: [{ $week: "$time_start" }, 1] },
                hour: {
                  $divide: [
                    { $subtract: ["$time_stop", "$time_start"] },
                    1000 * 60 * 60,
                  ],
                },
            ...
            {
              $group: {
                // group if user and week is the samen and sum the hours together
  ...
  for (let i = 0; i < user_assigned_project.length; i++) {
    for (let j = 0; j < user_assigned_project[i].sub_tasks.length; j++) {
      if (user_assigned_project[i].sub_tasks[j].clock_out_events) {
        for (
          let k = 0;
          k < user_assigned_project[i].sub_tasks[j].clock_out_events.length;
          k++
        ) {
          if (
            user_assigned_project[i].sub_tasks[j].clock_out_events[k].user_id
          ) {
            user_list.push(
              user_assigned_project[i].sub_tasks[j].clock_out_events[k].user_id
            );
